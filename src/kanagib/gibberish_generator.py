...
class UnigramGenerator:
    ...
    _DEFAULT_UNIGRAM_FILE = Path(__file__).parent / "data" / "livedoor_mora_unigram_frequency.json"
...
class BigramGenerator:
    ...
    _DEFAULT_BIGRAM_FILE = Path(__file__).parent / "data" / "livedoor_mora_bigram_freqeuncy.json"
    ...
    @staticmethod
    def _convert_bigram_to_conditional_probs(bigram_freq: dict[str, dict[str, int]]) -> dict[str, dict[str, float]]:
        """
        ...
        for w1, w2_counts in bigram_freq.items():
            ...
            if total_count > 0:
                conditional_probs[w1] = {w2: count / total_count for w2, count in w2_counts.items()}
            else:
                conditional_probs[w1] = {}
                
        return conditional_probs

    def generate(self, max_length: int = 100, stop_on_sep: bool = True, avoid_sep: bool = False) -> str:
        """
        ...
        for _ in range(max_length):
            ...
            if avoid_sep and "<SEP>" in next_mora_probs:
                filtered_probs = {k: v for k, v in next_mora_probs.items() if k != "<SEP>"}
                if filtered_probs:  # Ensure we have at least one option
                    ...
                    next_mora_probs = {k: v / total_prob for k, v in filtered_probs.items()}
