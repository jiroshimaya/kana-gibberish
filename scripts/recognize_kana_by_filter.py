...
def create_kana_vocabulary_mask(processor: AutoProcessor, model_name: str) -> torch.Tensor:
    ...
    for token, token_id in vocab.items():
        ...
        # トークンが全てかな文字でない場合は除外
        if check_token and not all(is_kana_character(c) or c in [' ', '　'] for c in check_token if c.strip()):
...
def setup_model_and_processor(model_name: str, device: torch.device) -> tuple[Wav2Vec2ForCTC, AutoProcessor]:
    """モデルとプロセッサーをセットアップ"""
...
def transcribe_audio(
    ...
    print(f"[INFO] Vocabulary restricted to kana characters. Allowed tokens: {vocab_mask.sum().item()}/{len(vocab_mask)}")
